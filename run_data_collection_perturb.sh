...
if [ $# != 2 ]; then
    echo "Usage: ./run_data_collection_perturb.sh <storage_path> <dataset_name>"
    exit 1
fi
...
python src/checkout_base_projects.py --storage $1 --$2 ${datasets[$2]} > $1/$2_checkout.out 2>&1
python src/filter_single_hunk.py --storage $1 --$2 ${datasets[$2]} --model_output $2_hunk.json --ignore_empty_diff > $1/$2_hunk.out 2>&1
