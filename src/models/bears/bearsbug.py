...
class BearsBug(Bug):
    ...
    def compile_impl(self) -> CompileResult:
        cmd = "cd %s; timeout 30 python2 compile_bug.py --bugId %s --workspace %s" % (self.path, pathlib.Path(self.path).name, pathlib.Path(self.path, ".."))
        run = subprocess.run(cmd, shell=True, capture_output=True)
        return CompileResult(True, run.returncode == 0)

    def test_impl(self) -> TestResult:
        cmd = "cd %s; timeout 120 python2 run_tests_bug.py --bugId %s --workspace %s" % (self.path, pathlib.Path(self.path).name, pathlib.Path(self.path, ".."))
