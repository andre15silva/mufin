...
if [ $# != 4 ]; then
    echo "Usage: ./run_generate_bugs_from_breaker.sh <storage_path> <dataset_name> <model_storage_path> <model_name>"
    exit 1
fi
...
python src/generate_bugs_from_breaker.py --storage $1 --$2 ${datasets[$2]} --from_pretrained $3/$4 --results_file X --beam_width 1 --model_input $2_hunk_compile_test.json --model_output $2_generated_bugs_$4.json > $1/$2_generated_bugs_$4.out 2>&1
